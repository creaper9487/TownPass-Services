  ...
  actions: {
    ...
    async rateOrganizer(uid: string,oid:string, rating: number,eventid:string) {
      try{
        ...
        const response = await fetch(`http://localhost:8000/api/organizer-ratings/`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    async getUserRatingForEvent(eventId: string, raterId: string) {
      try {
        const response = await fetch(`http://localhost:8000/api/organizer-ratings/event/${eventId}/rater/${raterId}`, {
          method: 'GET'
        });
        
        if (response.status === 404) {
          // User hasn't rated this event yet
          return null;
        }
        ...
        const data = await response.json();
        return data.rating; // Return the rating value
      } catch (error) {
